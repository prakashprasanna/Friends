...
import { Alert, List, ListItem, FlatList, TextInput, View, StyleSheet, Image, KeyboardAvoidingView, Text, ScrollView, ActivityIndicator, TouchableOpacity, TouchableHighlight, Picker, BackHandler, Switch, NetInfo, Dimensions, Platform, Animated, TouchableWithoutFeedback} from 'react-native';
...
import EditTimesheetEntry from './TimesheetEntry';
...
export default class FloatingTimesheet extends Component {
  ...
  if(!result.error){
   ...
   for(let t of result.data){
          ...
          if(t.start_time && t.end_time){
             temp[c] = {title:t.job_code+'     '+t.start_time_nice+' - '+t.end_time_nice+'                '+t.total_hours+'  '+t.num_units,date:t.timesheet_day,job_code:t.job_code,start_time:t.start_time,end_time:t.end_time,total_hours:t.total_hours,num_units:t.num_units,job_comment:t.job_comment,job_id:t.job_id,id:t.id,unpaid:t.unpaid_time}
          }else{
             temp[c] = {title:t.job_code+'                                               '+t.total_hours+'        '+t.num_units,date:t.timesheet_day,job_code:t.job_code,start_time:t.start_time,end_time:t.end_time,total_hours:t.total_hours,num_units:t.num_units,job_comment:t.job_comment,job_id:t.job_id,id:t.id,unpaid:t.unpaid_time}
          }
          that.setState({ 
            TimesheetDateSpecific: temp
          });          
...
this.props.navigation.push('EditTimesheet', {"date":moment(off[i].timesheet_day).format("dddd LL"),"jobs":off[i].job_id,"activity":off[i].job_code,"totalHours":off[i].total_hours,"comments":off[i].job_comment,"units":off[i].units,"email":this.props.navigation.state.params.email,"baseURL":this.props.navigation.state.params.baseURL,"user_id":this.props.navigation.state.params.user_id,'id':off[i].id,"error":null, "uuid":off[i].uuid, "startTime":off[i].start_time, "endTime":off[i].end_time, "unpaid":off[i].unpaid_time,"addPin":this.props.navigation.state.params.addPin},this.setState({ visibleModal: null }))
...
Dropdown: {
  ...
  offlineText: { color: '#fff',alignItems: 'center', },
  Mbutton: {
    backgroundColor: '#71BF44',
    padding: 12,
    margin: 16,
